...
@Component({
  templateUrl: './blockStaffList.component.html'
})

export class BlockStaffListComponent implements OnInit {
  ...
  getAllSubmission() {
    this.http.get("http://noticeboard.plenary-session.com/apis/" + "getBlockStaffList.php").subscribe(
      (res) => {
        if (res['length'] != 0) {
  ...
  this.http.post("http://noticeboard.plenary-session.com/apis/" + "activeStaff.php", payload).subscribe(
    (res) => {
