...
@Component({
  templateUrl: './blockTeacherList.component.html'
})

export class BlockTeacherListComponent implements OnInit {
  ...
  getAllSubmission() {
    this.http.get("http://noticeboard.plenary-session.com/apis/" + "getBlockTeacherList.php").subscribe(
      (res) => {
        if (res['length'] != 0) {
  ...
  this.http.post("http://noticeboard.plenary-session.com/apis/" + "activeTeacher.php", payload).subscribe(
    (res) => {
