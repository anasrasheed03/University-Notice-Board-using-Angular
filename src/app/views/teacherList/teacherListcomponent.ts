...
export class TeacherListComponent implements OnInit {
  ...
  editUserDetails(e){
    this.routr.navigate(['/teacher'], { queryParams: { id: e.id } });
}

getAllSubmission() {
  this.http.get("http://noticeboard.plenary-session.com/apis/" + "getTeacherList.php").subscribe(
    (res) => {
      if (res['length'] != 0) {
  ...
  disableTeacher(e){
    ...
    this.http.post("http://noticeboard.plenary-session.com/apis/" + "disableTeacher.php", payload).subscribe(
      (res) => {
