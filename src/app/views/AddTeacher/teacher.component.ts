...
export class TeacherComponent implements OnInit {
  ...
  postSubmission() {
      if(this.id == null){
    this.http.post("http://noticeboard.plenary-session.com/apis/" + "createTeacher.php", { data: this.simpleForm.value }).subscribe(
      (res) => {
    ...
    this.http.post("http://noticeboard.plenary-session.com/apis/" + "updateTeacher.php", payload).subscribe(
      (res) => {
        if (res['data']['status'] == 201) {
          this.onReset();
          this.toastr.success("", res['data']['message']);
  ...
  getSubmissionDataByID() {
    ...
    this.http.get("http://noticeboard.plenary-session.com/apis/" + "getTeacherByID.php?id=" + this.id).subscribe(
      (res) => {
        this.editCase= true;
         this.formData = res['data'];
          ...
          this.simpleForm.controls.facultyId.patchValue(res['data'][0].faculty_id);
          ...
          this.simpleForm.controls.designation.patchValue(res['data'][0].designation);
