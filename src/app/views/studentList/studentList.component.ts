...
export class StudentListComponent implements OnInit {
  ...
  getAllSubmission() {
    this.http.get("http://noticeboard.plenary-session.com/apis/" + "getStudentList.php").subscribe(
      (res) => {
        if (res['length'] != 0) {
  ...
  this.http.post("http://noticeboard.plenary-session.com/apis/" + "disableStudent.php", payload).subscribe(
    (res) => {
